    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        <div className="text-center">
          <div className={`text-3xl font-bold ${category.remaining < 0 ? 'text-red-600' : 'text-emerald-600'}`}>
      ...
      {/* Transactions list */}
      <div>
        <h4 className="text-sm font-medium text-slate-600 mb-2">עסקאות החודש</h4>
        {loading ? (
          <div className="flex justify-center py-6">
            <Loader2 className="h-6 w-6 animate-spin text-slate-400" />
          </div>
        ) : transactions.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-4">אין עסקאות בקטגוריה זו החודש</p>
        ) : (
          <div className="space-y-1 max-h-[40vh] overflow-y-auto">
            {transactions.map((tx) => (
              <div key={tx.id} className="flex items-center justify-between py-2 px-2 rounded hover:bg-slate-50">
                ...
                <span className={`text-sm font-bold shrink-0 mr-3 ${tx.type === 'INCOME' ? 'text-emerald-600' : 'text-red-600'}`}>
